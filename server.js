...
async function initDB() {
  ...
  await pool.query(`
    ...
    VALUES($1,$2,true)
    ON CONFLICT DO NOTHING`, ["admin", hash]);
...
app.post("/register", async (req,res)=>{
  ...
  try{ await pool.query("INSERT INTO users(username,password) VALUES($1,$2)",[username,hash]);
    res.json({success:true,message:"Registered successfully"});
  }catch{ res.json({success:false,message:"Username exists"}); }
});
...
app.post("/login", async (req,res)=>{
  ...
  if(!row || !bcrypt.compareSync(password,row.password)) return res.json({success:false,message:"Invalid login"});
...
app.post("/admin/delete-user", async (req,res)=>{
  ...
  const result = await pool.query("SELECT admin FROM users WHERE username=$1",[adminUsername]);
  if(!result.rows[0]?.admin) return res.json({success:false,message:"Unauthorized"});
  if(usernameToDelete===adminUsername) return res.json({success:false,message:"Cannot delete admin"});
  try{
    await pool.query("DELETE FROM users WHERE username=$1",[usernameToDelete]);
    await pool.query("DELETE FROM logins WHERE username=$1",[usernameToDelete]);
    res.json({success:true,message:`User ${usernameToDelete} deleted`});
  }catch(err){ console.error(err); res.json({success:false,message:"Error deleting user"}); }
});
...
wss.on("connection", ws=>{
  ...
  ws.on("message", async msg=>{
    try{
      ...
      if(data.type==="setUser"){ currentUser=data.username; clients.set(ws,currentUser); }
      if(data.type==="setAdmin"){ clients.set(ws,"admin"); broadcastActiveUsers(); }
      if(data.type==="chat"){
...
function broadcast(msg){
  ...
  clients.forEach((v,ws)=>{ if(ws.readyState===1) ws.send(str); });
}
